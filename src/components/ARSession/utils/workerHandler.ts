...
class WorkerHandler {
	...
	public postMessageToWorker(message: any) {
...
function workerFunction() {
	self.importScripts(
		'https://pub-dc411ce328ca438c8229e569659d6ebb.r2.dev/PosePlugin.js'
	);
	...
	function pluginInit(Module) {
		...
		function handleMessage(event: MessageEvent) {
			...
			if (type === 'Localize') {
				...
				const ptrPos = this._malloc(
					position.length * position.BYTES_PER_ELEMENT
				);
				this.HEAPF32.set(position, ptrPos >> 2);
				const ptrRot = this._malloc(
					rotation.length * rotation.BYTES_PER_ELEMENT
				);
				this.HEAPF32.set(rotation, ptrRot >> 2);
				const ptrIntrinsics = this._malloc(
					intrinsics.length * intrinsics.BYTES_PER_ELEMENT
				);
				this.HEAPF32.set(intrinsics, ptrIntrinsics >> 2);
				const ptrCamRot = this._malloc(
					camRot.length * camRot.BYTES_PER_ELEMENT
				);
				...
				if (r >= 0) {
					position = this.HEAPF32.subarray(
						ptrPos >> 2,
						(ptrPos >> 2) + position.length
					);
					rotation = this.HEAPF32.subarray(
						ptrRot >> 2,
						(ptrRot >> 2) + rotation.length
					);
					intrinsics = this.HEAPF32.subarray(
						ptrIntrinsics >> 2,
						(ptrIntrinsics >> 2) + intrinsics.length
					);
				}
