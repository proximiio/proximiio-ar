...
export const startStreams = (stream?: MediaStream) => {
	if (stream) {
	...
	Camera.Initialize(state.constraints)
		.then((camera: Camera) => gotStream(camera))
		.catch(handleError);
};
...
const gotStream = async (media: Camera): Promise<void> => {
	state.stream = media.el.srcObject as MediaStream; // make stream available to state
	state.$video = media.el as HTMLVideoElement;
	...
	state.alva = await AlvaAR.Initialize(
		state.$canvas.width,
		state.$canvas.height
	);
	state.alvaView = new ARCamView(
		state.$view,
		state.$canvas.width,
		state.$canvas.height
	);
	...
	// Load a texture
	textureLoader.load(
		new URL('https://pub-dc411ce328ca438c8229e569659d6ebb.r2.dev/ad.jpg').href,
		(texture: Texture) => {
	...
	// Load a font
	fontLoader.load(
		new URL('https://pub-dc411ce328ca438c8229e569659d6ebb.r2.dev/helvetiker_regular.typeface.json')
			.href,
		(font) => {
			...
			const textGeometry = new TextGeometry('Take me there', {
				...
				bevelEnabled: true, // Enable bevel to add depth to the text
...
const handleError = (error: { message: string; name: string }) => {
	console.log(
